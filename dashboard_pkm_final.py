...
# ==============================================================================
# 1. VISUAL STYLE (HUD DARK MODE - SESUAI REQUEST)
# ==============================================================================
st.set_page_config(
    page_title="EdgeGuard Commander v3.0",
    page_icon="💠",
    layout="wide",
...
# ==============================================================================
# 2. PHYSICS ENGINE (DATA DINAMIS & TERKORELASI)
# ==============================================================================
@st.cache_data
def get_smart_dataset():
    ...
    # 1. HIDROLIK (ARUS AIR)
    # Pola: Hujan (Tinggi), Kemarau (Rendah), plus fluktuasi harian
    t = np.linspace(0, 4*np.pi, hours)
    flow = 0.5 + 0.35 * np.sin(t) + np.random.normal(0, 0.05, hours)
    flow = np.clip(flow, 0.05, 0.95) # m/s
    
    # 2. TURBIN (MEKANIK -> ELEKTRIK)
    # Potensi Alam (Teoritis): P ~ v^3
    p_theory = 0.5 * 1000 * 2.0 * (flow**3)
    
    # Efisiensi Turbin Dinamis (PR): 
    # Turbin tidak efisien di arus rendah (<0.2 m/s). Eff naik seiring arus naik.
    # Rumus kurva efisiensi logistik sederhana
    ...
    # PR rendah jika arus rendah (karena gesekan mekanik dominan)
    pr_turbin = turbin_eff_curve * 100 
    
    # RPM (Linear terhadap flow)
    rotor_rpm = flow * 65
    gen_rpm = rotor_rpm * 15 # Gearbox 1:15
    
    # 3. ELECTRODIALYSIS (LOAD & QUALITY)
    # Tegangan Generator (Volt) sebanding dengan RPM/Power
    voltage = (p_gen / 55) + 10 # Baseline 10V
    voltage = np.clip(voltage, 0, 48)
    
    # Logic Kualitas & Konsumsi (Saling Terkait)
    p_load = []
    rejection = []
    status = []
    ...
    for v, p_avail in zip(voltage, p_gen):
        if v < 12:
            ...
            cons = 5 # Watt (idle)
            rej = 0  # Tidak ada pemisahan
            e_spec = 0
        elif v < 22:
            # Mode  (Normal)
            mode = "Berjalan Sedang / Normal"
            cons = p_avail * 0.6 # Pakai 60% daya
            rej = 70 + (v-12)*2.5 # Kualitas naik linear (70-95%)
            e_spec = 0.8
        else:
            mode = "Berjalan Optimal"
            cons = p_avail * 0.85 # Pakai 85% daya
            rej = 98.0 + np.random.uniform(0, 1.5) # Stabil tinggi
            e_spec = 1.8 # Boros energi tapi bersih
            
        p_load.append(cons)
        rejection.append(min(99.9, rej))
        status.append(mode)
        sec.append(e_spec)
    ...
    df = pd.DataFrame({
        'Timestamp': dates,
        'Formatted_Date': dates.strftime('%d %b %Y %H:%00'), # String untuk Slider
...
with c_head1:
    st.markdown("## 💠 EDGEGUARD COMMANDER v3.0")
    st.caption("Integrated Hybrid Energy & Desalination Analytics")
...
if view_mode == "🔴 REAL-TIME (Harian)":
    ...
    # 1. SLIDER TANGGAL ASLI (Bukan Angka)
    st.markdown("### ⏳ TIMELINE OPERASIONAL")
    
    # Menggunakan select_slider agar yang muncul TANGGAL, bukan angka 0-8000
    time_options = df['Formatted_Date'].tolist()
    # Default start di tengah tahun
    ...
    # --- PANEL PENJELASAN DINAMIS (SYSTEM INSIGHT) ---
    # Logic Pembuatan Kalimat Penjelasan
    ...
    <div class="insight-box">
        <div class="insight-title">🤖 SYSTEM INSIGHT (ANALISIS OTOMATIS)</div>
        ...
        Hasilnya, kualitas pemisahan garam mencapai <b>{row['Rejection']:.2f}%</b> dengan sisa energi (surplus) sebesar <b>{row['P_Surplus']:.0f} Watt</b> untuk baterai.
    </div>
    """, unsafe_allow_html=True)

    # --- SECTION 1: NERACA ENERGI & PERFORMA ---
    ...
    # Performance Ratio Turbin (Dinamis sesuai arus)
    pr_color = "#4ade80" if row['PR_Turbin'] > 30 else "#f87171"
    hud_card(k4, "PR TURBIN", f"{row['PR_Turbin']:.1f}", "% EFISIENSI", pr_color)

    # --- SECTION 2: PERFORMA HYBRID ELECTRODIALYSIS ---
    st.markdown('<div class="section-header">2. Performa Hybrid Multicell ED</div>', unsafe_allow_html=True)
    
    g1, g2, g3 = st.columns([1, 1, 2])
    
    with g1:
        # Gauge Salt Rejection (Kualitas Air)
        fig_rej = go.Figure(go.Indicator(
            mode = "gauge+number", value = row['Rejection'],
            title = {'text': "Salt Rejection (%)", 'font': {'size': 14}},
            gauge = {
                'axis': {'range': [0, 100]}, 
                'bar': {'color': "#4ade80" if row['Rejection'] > 95 else "#f87171"},
    ...
    with g3:
        # Grafik Korelasi Arus vs Rejection (Real-time Zoom)
        ...
        fig_trend.add_trace(go.Scatter(x=df_zoom['Timestamp'], y=df_zoom['Rejection'], name='Rejection (%)', yaxis='y2', line=dict(color='#4ade80')))
        ...
        # FIX ERROR 'titlefont' -> gunakan dictionary yang benar
        fig_trend.update_layout(
            template="plotly_dark", height=200,
            yaxis=dict(title=dict(text="Arus (m/s)", font=dict(color="#00f2ff"))),
            yaxis2=dict(title=dict(text="Rejection (%)", font=dict(color="#4ade80")), overlaying='y', side='right'),
    ...
    # --- LOGIC UNTUK BULANAN / TAHUNAN ---
    c_filt1, c_filt2 = st.columns(2)
    with c_filt1:
        agg_method = st.selectbox("METODE KALKULASI:", ["Rata-rata (Average)", "Total (Sum)", "Maksimum (Max)"])
    ...
    if "Total" in agg_method:
        ...
        row_data['Rejection'] = df_target['Rejection'].mean()
